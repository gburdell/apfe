...
module_declaration :
module_ansi_header timeunits_declaration? non_port_module_item* ENDMODULE_K (COLON module_identifier)?
| module_nonansi_header timeunits_declaration? module_item* ENDMODULE_K (COLON module_identifier)?
...
interface_declaration :
interface_ansi_header timeunits_declaration? non_port_interface_item* ENDINTERFACE_K (COLON interface_identifier)?
| interface_nonansi_header timeunits_declaration? interface_item* ENDINTERFACE_K (COLON interface_identifier)?
| attribute_instance* INTERFACE_K interface_identifier ( DOT_STAR ) SEMI timeunits_declaration? interface_item* ENDINTERFACE_K 
...
attribute_instance* INTERFACE_K lifetime? interface_identifier package_import_declaration* parameter_port_list? 
	list_of_ports SEMI
;
...
program_declaration :
program_ansi_header timeunits_declaration? non_port_program_item* ENDPROGRAM_K (COLON program_identifier)?
| program_nonansi_header timeunits_declaration? program_item* ENDPROGRAM_K (COLON program_identifier)?
| attribute_instance* PROGRAM_K program_identifier ( DOT_STAR ) SEMI timeunits_declaration? program_item* ENDPROGRAM_K 
...
attribute_instance* PROGRAM_K lifetime? program_identifier package_import_declaration* parameter_port_list? 
	list_of_ports SEMI
;
...
checker_declaration :
CHECKER_K checker_identifier (LPAREN checker_port_list? RPAREN)? SEMI checker_or_generate_item* ENDCHECKER_K 
...
(net_port_header | interface_port_header)? port_identifier unpacked_dimension* ( EQ constant_expression )?
...
config_declaration :
CONFIG_K config_identifier SEMI (local_parameter_declaration SEMI)* design_statement config_rule_statement*
...
use_clause : 
  USE_K (library_identifier DOT)? cell_identifier named_parameter_assignment (COMMA named_parameter_assignment)* (COLON CONFIG_K)?
| USE_K named_parameter_assignment (COMMA named_parameter_assignment)* (COLON CONFIG_K)?
| USE_K (library_identifier DOT)? cell_identifier (COLON CONFIG_K)?
;
...
checker_port_item :
attribute_instance* property_formal_type port_identifier variable_dimension* (EQ property_actual_arg)?
;
...
enum_name_declaration : enum_identifier (LBRACK integral_number (COLON integral_number)? RBRACK)? (EQ constant_expression)?
;

class_scope : class_type COLON2
;

class_type : ps_class_identifier parameter_value_assignment?  (COLON2 class_identifier parameter_value_assignment?)*
;
...
list_of_interface_identifiers : interface_identifier param_assignment* (COMMA interface_identifier param_assignment*)*
;
...
list_of_port_identifiers : port_identifier param_assignment* (COMMA port_identifier param_assignment*)*
;
...
port_identifier variable_dimension* (EQ expression)?  (COMMA port_identifier variable_dimension* (EQ expression)?)*
;
...
port_identifier variable_dimension* (EQ constant_expression)?  (COMMA port_identifier variable_dimension* (EQ constant_expression)?)*
;

list_of_virtual_interface_decl :
variable_identifier (EQ interface_instance_identifier)?  (COMMA variable_identifier (EQ interface_instance_identifier)?)*
;
...
param_assignment : parameter_identifier unpacked_dimension* (EQ constant_param_expression)?
;
...
sequence_method_call : sequence_instance DOT method_identifier
;
...
let_port_item : ( attribute_instance )* let_formal_type port_identifier ( variable_dimension )* ( EQ expression )?
;
...
| ( WILDCARD_K )? bins_keyword bin_identifier ( LBRACK ( expression )? RBRACK )? EQ 
...
cover_cross : ( cross_identifier COLON )? CROSS_K list_of_coverpoints ( IFF_K LPAREN expression RPAREN )? select_bins_or_empty
;
...
pass_enable_switch_instance : ( name_of_instance )? LPAREN inout_terminal COMMA inout_terminal COMMA enable_terminal RPAREN
;
...
udp_nonansi_declaration : ( attribute_instance )* PRIMITIVE_K udp_identifier LPAREN udp_port_list RPAREN SEMI
;

udp_ansi_declaration : ( attribute_instance )* PRIMITIVE_K udp_identifier LPAREN udp_declaration_port_list RPAREN SEMI
;
...
| ( implicit_class_handle DOT | class_scope | package_scope )? hierarchical_variable_identifier select EQ class_new
...
seq_block :
BEGIN_K ( COLON block_identifier )? ( block_item_declaration )* ( statement_or_null )* END_K ( COLON block_identifier )?
;

par_block :
FORK_K ( COLON block_identifier )? ( block_item_declaration )* ( statement_or_null )* join_keyword ( COLON block_identifier )?
;
...
| (DEFAULT_K)? CLOCKING_K ( clocking_identifier )? clocking_event SEMI ( clocking_item )* ENDCLOCKING_K ( COLON clocking_identifier )?
;
...
//A.6.12 Randsequence
randsequence_statement : RANDSEQUENCE_K LPAREN ( production_identifier )? RPAREN production ( production )* ENDSEQUENCE_K
;
...
delayed_data : terminal_identifier (LBRACK constant_mintypmax_expression RBRACK)? // | terminal_identifier
;

delayed_reference : terminal_identifier (LBRACK constant_mintypmax_expression RBRACK)?  // | terminal_identifier
;
...
scalar_timing_check_condition :
  expression EQ2 SCALAR_CONTENT
| expression EQ3 SCALAR_CONTENT
| expression NOT_EQ SCALAR_CONTENT
| expression NOT_EQ2 SCALAR_CONTENT
| TILDE? expression
;

SCALAR_CONTENT : 
// \u0027 is single quoute (apostrophe)
...
array_method_name : method_identifier | UNIQUE_K | AND_K | OR_K | XOR_K
;
...
tagged_union_expression : TAGGED_K member_identifier ( expression )?
;
...
//ILR{ | casting_type 
//FIXMEv4:  (simple_type | constant_primary | signing | STRING_K | CONST_K)
(simple_type | /*constant_primary |*/ signing | STRING_K | CONST_K)
//}
	SQUOTE LPAREN constant_expression RPAREN
//}
...
select : ( ( DOT member_identifier bit_select )* DOT member_identifier )? bit_select ( LBRACK part_select_range RBRACK )?
;

nonrange_select : ( ( DOT member_identifier bit_select )* DOT member_identifier )? bit_select
;
...
unary_module_path_operator : NOT | TILDE | AND | TILDE_AND | OR | TILDE_OR | XOR | TILDE_XOR | XOR_TILDE
;

binary_module_path_operator : EQ2 | NOT_EQ | AND2 | OR2 | AND | OR | XOR | XOR_TILDE | TILDE_XOR
;
...
ACCEPT_ON_K  :  'accept_on'   ;
ALIAS_K  :  'alias'   ;
ALWAYS_COMB_K  :  'always_comb'   ;
ALWAYS_FF_K  :  'always_ff'   ;
ALWAYS_K  :  'always'   ;
ALWAYS_LATCH_K  :  'always_latch'   ;
AND_K  :  'and'   ;
ASSERT_K  :  'assert'   ;
ASSIGN_K  :  'assign'   ;
ASSUME_K  :  'assume'   ;
AUTOMATIC_K  :  'automatic'   ;
BEFORE_K  :  'before'   ;
BEGIN_K  :  'begin'   ;
BIND_K  :  'bind'   ;
BINSOF_K  :  'binsof'   ;
...
CMOS_K  :  'cmos'   ;
CONFIG_K  :  'config'   ;
CONSTRAINT_K  :  'constraint'   ;
CONST_K  :  'const'   ;
CONTEXT_K  :  'context'   ;
CONTINUE_K  :  'continue'   ;
COVERGROUP_K  :  'covergroup'   ;
COVERPOINT_K  :  'coverpoint'   ;
COVER_K  :  'cover'   ;
CROSS_K  :  'cross'   ;
DEASSIGN_K  :  'deassign'   ;
DEFAULT_K  :  'default'   ;
...
ELSE_K  :  'else'   ;
ENDCASE_K  :  'endcase'   ;
ENDCHECKER_K  :  'endchecker'   ;
ENDCLASS_K  :  'endclass'   ;
ENDCLOCKING_K  :  'endclocking'   ;
ENDCONFIG_K  :  'endconfig'   ;
ENDFUNCTION_K  :  'endfunction'   ;
ENDGENERATE_K  :  'endgenerate'   ;
ENDGROUP_K  :  'endgroup'   ;
ENDINTERFACE_K  :  'endinterface'   ;
ENDMODULE_K  :  'endmodule'   ;
ENDPACKAGE_K  :  'endpackage'   ;
ENDPRIMITIVE_K  :  'endprimitive'   ;
ENDPROGRAM_K  :  'endprogram'   ;
ENDPROPERTY_K  :  'endproperty'   ;
ENDSEQUENCE_K  :  'endsequence'   ;
ENDSPECIFY_K  :  'endspecify'   ;
ENDTABLE_K  :  'endtable'   ;
ENDTASK_K  :  'endtask'   ;
END_K  :  'end'   ;
ENUM_K  :  'enum'   ;
EVENTUALLY_K  :  'eventually'   ;
EVENT_K  :  'event'   ;
EXPECT_K  :  'expect'   ;
EXPORT_K  :  'export'   ;
EXTENDS_K  :  'extends'   ;
...
IFNONE_K  :  'ifnone'   ;
IF_K  :  'if'   ;
IGNORE_BINS_K  :  'ignore_bins'   ;
ILLEGAL_BINS_K  :  'illegal_bins'   ;
IMPLIES_K  :  'implies'   ;
IMPORT_K  :  'import'   ;
INCDIR_K  :  'incdir'   ;
INCLUDE_K  :  'include'   ;
...
INSTANCE_K  :  'instance'   ;
INTEGER_K  :  'integer'   ;
INTERFACE_K  :  'interface'   ;
INTERSECT_K  :  'intersect'   ;
INT_K  :  'int'   ;
JOIN_ANY_K  :  'join_any'   ;
JOIN_K  :  'join'   ;
JOIN_NONE_K  :  'join_none'   ;
LARGE_K  :  'large'   ;
LET_K  :  'let'   ;
LIBLIST_K  :  'liblist'   ;
LIBRARY_K  :  'library'   ;
LOCALPARAM_K  :  'localparam'   ;
LOCAL_K  :  'local'   ;
LOGIC_K  :  'logic'   ;
LONGINT_K  :  'longint'   ;
MACROMODULE_K  :  'macromodule'   ;
MATCHES_K  :  'matches'   ;
MEDIUM_K  :  'medium'   ;
MODPORT_K  :  'modport'   ;
MODULE_K  :  'module'   ;
...
NMOS_K  :  'nmos'   ;
NOR_K  :  'nor'   ;
NOSHOWCANCELLED_K  :  'noshowcancelled'   ;
NOTIF0_K  :  'notif0'   ;
NOTIF1_K  :  'notif1'   ;
NOT_K  :  'not'   ;
NULL_K  :  'null'   ;
...
PACKAGE_K  :  'package'   ;
PACKED_K  :  'packed'   ;
PARAMETER_K  :  'parameter'   ;
PMOS_K  :  'pmos'   ;
POSEDGE_K  :  'posedge'   ;
PRIMITIVE_K  :  'primitive'   ;
PRIORITY_K  :  'priority'   ;
PROGRAM_K  :  'program'   ;
PROPERTY_K  :  'property'   ;
...
PULLDOWN_K  :  'pulldown'   ;
PULLUP_K  :  'pullup'   ;
PULSESTYLE_ONDETECT_K  :  'pulsestyle_ondetect'   ;
PULSESTYLE_ONEVENT_K  :  'pulsestyle_onevent'   ;
PURE_K  :  'pure'   ;
RANDCASE_K  :  'randcase'   ;
RANDC_K  :  'randc'   ;
RANDSEQUENCE_K  :  'randsequence'   ;
RAND_K  :  'rand'   ;
RANDOMIZE_K  :  'randomize'   ;
RCMOS_K  :  'rcmos'   ;
REALTIME_K  :  'realtime'   ;
REAL_K  :  'real'   ;
REF_K  :  'ref'   ;
REG_K  :  'reg'   ;
REJECT_ON_K  :  'reject_on'   ;
RELEASE_K  :  'release'   ;
REPEAT_K  :  'repeat'   ;
RESTRICT_K  :  'restrict'   ;
RETURN_K  :  'return'   ;
...
SEQUENCE_K  :  'sequence'   ;
SHORTINT_K  :  'shortint'   ;
SHORTREAL_K  :  'shortreal'   ;
SHOWCANCELLED_K  :  'showcancelled'   ;
SIGNED_K  :  'signed'   ;
SMALL_K  :  'small'   ;
SOLVE_K  :  'solve'   ;
SPECIFY_K  :  'specify'   ;
SPECPARAM_K  :  'specparam'   ;
STATIC_K  :  'static'   ;
STRING_K  :  'string'   ;
STRONG0_K  :  'strong0'   ;
STRONG1_K  :  'strong1'   ;
...
SUPPLY0_K  :  'supply0'   ;
SUPPLY1_K  :  'supply1'   ;
SYNC_ACCEPT_ON_K  :  'sync_accept_on'   ;
SYNC_REJECT_ON_K  :  'sync_reject_on'   ;
S_ALWAYS_K  :  's_always'   ;
S_EVENTUALLY_K  :  's_eventually'   ;
S_NEXTTIME_K  :  's_nexttime'   ;
S_UNTIL_K  :  's_until'   ;
S_UNTIL_WITH_K  :  's_until_with'   ;
TABLE_K  :  'table'   ;
TAGGED_K  :  'tagged'   ;
TASK_K  :  'task'   ;
THIS_K  :  'this'   ;
THROUGHOUT_K  :  'throughout'   ;
TIMEPRECISION_K  :  'timeprecision'   ;
TIMEUNIT_K  :  'timeunit'   ;
TIME_K  :  'time'   ;
TRANIF0_K  :  'tranif0'   ;
TRANIF1_K  :  'tranif1'   ;
...
XNOR_K  :  'xnor'   ;
XOR_K  :  'xor'   ;
W1STEP_K  :  '1step' 	//NOTE: actually not a keyword in LRM
;
...
COLON_EQ : 			':='	 ;
COMMA : 				','		 ;
//changed to NUMBER :  DIGIT_012:			'0'..'2' ;
DIV : 				'/'		 ;
DIV_EQ : 				'/='	 ;
DOLLAR : 				'$'		 ;
DOT : 				'.'		 ;
...
PATHPULSE_K : 	'PATHPULSE$' ;

DS_SETUP_K : 		'$setup' ;
DS_REMOVAL_K :  	'$removal' ;
DS_PERIOD_K : 		'$period' ;
DS_RECREM_K : 		'$recrem' ;
DS_SETUPHOLD_K : 	'$setuphold' ;
DS_RECOVERY_K : 	'$recovery' ;
DS_HOLD_K : 		'$hold' ;
DS_WIDTH_K : 		'$width' ;
DS_SKEW_K : 		'$skew' ;
DS_FULLSKEW_K : 	'$fullskew' ;
DS_TIMESKEW_K : 	'$timeskew' ;
DS_NOCHANGE_K : 	'$nochange' ;
DS_FATAL_K : 		'$fatal' ;
DS_ERROR_K : 		'$error' ;
DS_WARNING_K : 		'$warning' ;
DS_INFO_K : 		'$info' ;
DS_UNIT_K : 		'$unit' ;
DS_ROOT_K : 		'$root' ;
